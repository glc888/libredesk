<template>
	<div class="flex items-center justify-center gap-3">
		...
		<div class="flex flex-col">
          ...
          ></span> -->
					<span class="inline-block w-2 h-2 rounded-full mr-1 bg-success"></span>
...
<script setup>
	import {
		useChatStore
	} from '@widget/store/chat.js'
	import {
		useWidgetStore
	} from '@widget/store/widget.js'
	import {
		computed
	} from 'vue'
	import {
	...
	import {
		getRelativeTime
	} from '@shared-ui/utils/datetime.js'
	import {
		useBusinessHours
	} from '@widget/composables/useBusinessHours.js'
	import {
		useI18n
	} from 'vue-i18n'
	...
	const businessHoursStatus = computed(() => {
		...
		const {
			status,
			isWithin
		} = getBusinessHoursStatus(businessHours, utcOffset, withinHoursMessage)
		if (!isWithin) {
			return status
		}
